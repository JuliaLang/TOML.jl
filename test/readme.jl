...
# Some of the tests from the earlier TOML.jl package
...
l = Parser("true")
@test parse_value(l) == true
l = Parser("false")
@test parse_value(l) == false
l = Parser("inf")
@test parse_value(l) == Inf
l = Parser("-inf")
@test parse_value(l) == -Inf
l = Parser("nan")
@test isnan(parse_value(l))
l = Parser("-nan")
@test isnan(parse_value(l))
l = Parser("[true, false]")
@test parse_value(l) == [true, false]
l = Parser("[true, # this is a comment
...
l = Parser("foo.bar = true")
k, v = parse_entry(l, l.root)
@test v == true
@test k.value == "foo"
@test k.next.value == "bar"

l = Parser("123.  456 = true")
k, v = parse_entry(l, l.root)
@test v == true
@test k.value == "123"
@test k.next.value == "456"

l = Parser(""""foo" """)
@test parse_value(l) == "foo"


l = Parser("""{foo = "bar", x = true}""")
@test parse_value(l) == Dict("foo" => "bar", "x" => true)
...
err = tryparsestring(str)
@test err isa ParserError
@test err.type == Internals.ErrUnexpectedEndString
